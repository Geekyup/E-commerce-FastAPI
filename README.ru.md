...
- ⭐ **Отзывы о товарах** — Пользовательские отзывы с оценками 1-5 звёзд и расчётом среднего рейтинга
- 🛒 **Корзина** — Персональная корзина с управлением позициями
- 📦 **Заказы** — Управление заказами с отслеживанием статуса
- 🗄️ **Миграции БД** — Версионирование схемы через Alembic
- ⚙️ **Админ-панель** — Встроенное управление суперпользователями с FastAdmin
...
| `POST` | `/api/reviews/` | Создать отзыв о товаре |
| `GET` | `/api/reviews/product/{product_id}` | Получить отзывы о товаре |
| `GET` | `/api/reviews/user/{user_id}` | Получить отзывы пользователя |
| `PATCH` | `/api/reviews/{review_id}` | Обновить отзыв |
